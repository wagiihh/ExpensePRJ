...
if (isset($_POST['add'])) {
    ...
    $query = "INSERT INTO expenses (user_id, expense, expensedate, expensecategory) VALUES ('$userid', '$expenseamount', '$expensedate', '$expensecategory')";
...
if (isset($_POST['update']) && $id) {
    ...
    $query = "UPDATE expenses SET expense='$expenseamount', expensedate='$expensedate', expensecategory='$expensecategory' WHERE user_id='$userid' AND expense_id='$id'";
    if (!mysqli_query($con, $query)) {
...
if (isset($_POST['delete']) && $id) {
    $query = "DELETE FROM expenses WHERE user_id='$userid' AND expense_id='$id'";
    if (!mysqli_query($con, $query)) {
        die("ERROR: Could not execute $query. " . mysqli_error($con));
    }
    header('location: manage_expense.php');
...
// Fetch Record for Edit/Delete
if ($id && (isset($_GET['edit']) || isset($_GET['delete']))) {
    ...
    if (mysqli_num_rows($record) == 1) {
        ...
        $update = isset($_GET['edit']);
        $del = isset($_GET['delete']);
    } else {
